 ...
 * This block handles the differences between the Canvas sandbox and Vercel/Production.
 */
const getSafeConfig = () => {
  let config = {
    firebaseConfig: null,
  ...
  // 1. Handle Canvas Sandbox Globals
  if (typeof __firebase_config !== 'undefined' && __firebase_config) {
    try {
      ...
      console.error("Sandbox config parse error");
    }
  }

  // 2. Handle Vite/Vercel Environment Variables
  try {
    // @ts-ignore - Ignore check for import.meta in environments that don't support it
    const metaEnv = typeof import.meta !== 'undefined' && import.meta.env ? import.meta.env : null;
    if (metaEnv) {
      if (metaEnv.VITE_FIREBASE_CONFIG) {
        config.firebaseConfig = typeof metaEnv.VITE_FIREBASE_CONFIG === 'string' ? JSON.parse(metaEnv.VITE_FIREBASE_CONFIG) : metaEnv.VITE_FIREBASE_CONFIG;
      }
      if (metaEnv.VITE_APP_ID) {
        config.appId = metaEnv.VITE_APP_ID;
      }
      if (metaEnv.VITE_GEMINI_API_KEY) {
        config.geminiKey = metaEnv.VITE_GEMINI_API_KEY;
      }
    }
  } catch (e) {}

  // CRITICAL: Sanitize appId for Firestore (must not contain slashes or it breaks path segment counts)
...
export default function App() {
    ...
    const initAuth = async () => {
      ...
      if (!auth.currentUser) {
        try {
          ...
          setError("Database restricted. Check Firebase Auth settings.");
        }
      }
    };
    initAuth();
  ...
  useEffect(() => {
    if (!user || !db) return;
    // Strict path: /artifacts/{sanitized_appId}/users/{userId}/timelines
    ...
    const unsubscribe = onSnapshot(libraryCol, (snapshot) => {
      ...
      console.error("Cloud Library error:", err);
    });
    return () => unsubscribe();
  }, [user]);
  ...
  const loadFromSheet = async (url) => {
    ...
    } catch (err) { setError("Google Sheet load failed."); }
  ...
  const handleAIGeneration = async () => {
    ...
    const apiKey = ""; 
    const effectiveKey = (apiKey === "" && geminiKey) ? geminiKey : apiKey;
    if (!effectiveKey) {
    ...
    const prompt = `Timeline: "${aiTopic}". 35 events. Return JSON: { "events": [{ "date": "YYYY-MM-DD", "title": "string", "description": "string", "imageurl": "Wikimedia file URL", "importance": 1-10 }] }`;
  ...
  const handleShare = () => {
    ...
    } catch (e) { setError("Sharing not supported."); }
  };
      ...
      <div className="h-screen bg-slate-50 flex flex-col items-center justify-center p-8 text-center">
        <div className="bg-white p-12 rounded-[3rem] shadow-2xl max-w-md border border-slate-100">
          ...
          <p className="text-slate-500 mb-8 leading-relaxed text-center">
            The application is ready but needs your Firebase configuration. Follow the guide to finish setup.
          </p>
          <div className="bg-slate-50 p-4 rounded-2xl text-[10px] font-mono text-left border border-slate-100 opacity-60">
            Path segment check: {appId}
          </div>
        </div>
      </div>
    );
    ...
    <div className="h-screen bg-[#fafaf9] text-slate-900 font-sans flex flex-col overflow-hidden selection:bg-blue-100">
      ...
      <header className="bg-white/90 backdrop-blur-md border-b border-slate-200 px-6 py-4 z-50 shrink-0 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowLibrary(!showLibrary)} className={`p-2.5 rounded-full border transition-all ${showLibrary ? 'bg-slate-900 text-white shadow-lg' : 'bg-white text-slate-600 hover:bg-slate-50'}`} title="Library"><FolderOpen size={18} /></button>
            <button onClick={() => setShowSaveDialog(true)} disabled={events.length === 0} className="p-2.5 rounded-full border bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-30 transition-all shadow-sm" title="Save"><Save size={18} /></button>
            <button onClick={handleShare} className="p-2.5 rounded-full border bg-white text-slate-600 hover:bg-slate-50 transition-all shadow-sm" title="Share Link">
        ...
        {showSettings && (
          <div className="max-w-7xl mx-auto mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4 animate-in slide-in-from-top-4 duration-300">
            <input type="text" placeholder="Google Sheet Link..." value={sheetUrl} onChange={(e)=>setSheetUrl(e.target.value)} onKeyDown={(e)=>e.key==='Enter'&&loadFromSheet(sheetUrl)} className="flex-1 px-4 py-3 bg-slate-50 border rounded-2xl text-sm font-mono outline-none shadow-inner"/>
            <div className="flex gap-2">
              <input type="text" placeholder="AI Topic..." value={aiTopic} onChange={(e)=>setAiTopic(e.target.value)} onKeyDown={(e)=>e.key==='Enter'&&handleAIGeneration()} className="flex-1 px-4 py-3 bg-blue-50/30 border border-blue-100 rounded-2xl text-sm outline-none shadow-inner"/>
              <button onClick={handleAIGeneration} disabled={loading} className="px-6 py-3 bg-blue-600 text-white rounded-2xl font-bold shadow-md active:scale-95 transition-all">Generate</button>
            </div>
          </div>
        )}

        {showLibrary && (
          <div className="max-w-7xl mx-auto mt-6 p-6 bg-slate-900 rounded-[2.5rem] shadow-2xl animate-in slide-in-from-top-4 duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-[40vh] overflow-y-auto custom-scrollbar p-1">
              {savedTimelines.length === 0 ? (
                <div className="col-span-full py-12 text-center text-slate-500 text-sm italic">Library is empty.</div>
              ) : savedTimelines.map(tl => (
                <div key={tl.id} onClick={()=>loadTimelineFromLibrary(tl)} className="bg-slate-800 hover:bg-slate-700 p-5 rounded-2xl cursor-pointer transition-all border border-slate-700 group relative shadow-md">
                  <div className="flex justify-between items-start mb-3">
                    <span className="text-white font-bold text-sm leading-tight line-clamp-1">{String(tl.name)}</span>
                    <button onClick={(e)=>deleteTimeline(tl.id, e)} className="text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all p-1"><Trash2 size={14}/></button>
                  </div>
                  <div className="text-[10px] text-slate-500 flex justify-between uppercase font-bold tracking-widest">
                    <span>{Number(tl.events?.length || 0)} Points</span>
        ...
        {(String(error) || String(statusMessage) || loading) && (
          <div className="max-w-7xl mx-auto mt-4">
            {error && <div className="text-sm text-red-600 flex items-center gap-2 bg-red-50 p-4 rounded-2xl border border-red-100 animate-in fade-in"><AlertCircle size={18} /> {String(error)}</div>}
            {statusMessage && <div className="text-sm text-blue-700 flex items-center gap-2 bg-blue-50 p-4 rounded-2xl border border-blue-100 shadow-sm animate-in fade-in"><CheckCircle2 size={18}/> {String(statusMessage)}</div>}
            {loading && !error && !statusMessage && <div className="text-sm text-blue-700 flex items-center gap-3 bg-blue-50/50 p-4 rounded-2xl animate-pulse"><RefreshCw size={18} className="animate-spin"/> Researching...</div>}
          </div>
        )}
      </header>
      ...
      <main className="flex-1 relative overflow-hidden flex flex-col">
              ...
              {layoutItems.map((item) => (
                item.type === 'marker' ? (
                  <div key={item.id} className="relative flex flex-col items-center justify-end w-24 shrink-0">
                    ...
                    <div className="absolute -bottom-12 text-[10px] font-black text-slate-400 tracking-[0.3em]">{Number(item.year)}</div>
                  </div>
                ) : (
                  <div key={item.id} ref={el => eventRefs.current[item.id] = el} data-date={new Date(item.data.date).toLocaleDateString('en-US',{month:'short',year:'numeric'})} className="relative flex flex-col items-center justify-end w-[320px] md:w-[420px] shrink-0 snap-center group">
                    <div className="w-full bg-white rounded-[3rem] border border-slate-100 shadow-2xl transition-all duration-700 overflow-hidden flex flex-col mb-16 relative z-20 hover:-translate-y-8 hover:shadow-[0_40px_80px_rgba(0,0,0,0.1)] transition-all duration-500">
                      <div className="h-48 md:h-64 overflow-hidden relative bg-slate-100 flex items-center justify-center">
                        <ImageWithFallback src={item.data.imageurl} alt={String(item.data.title)} />
                      </div>
                      <div className="p-10">
                        <div className="flex justify-between items-center mb-6">
                          ...
                          <span className="text-[10px] font-bold text-blue-500">Imp. {Number(item.data.importance)}</span>
                        </div>
                        <h3 className="font-serif font-bold text-2xl md:text-3xl text-slate-900 leading-[1.15] mb-6 group-hover:text-blue-600 transition-colors line-clamp-2">
        ...
        <div className="absolute bottom-8 left-10 right-10 pointer-events-none flex justify-between items-center z-40">
           <div className="bg-white/95 backdrop-blur p-4 rounded-2xl shadow-xl border border-slate-100 flex items-center gap-3 text-[10px] font-black tracking-widest text-slate-400 uppercase pointer-events-auto cursor-default shadow-md">
             <ChevronLeft size={16}/> Glide Through Time
           </div>
           <div className="bg-white/95 backdrop-blur p-4 rounded-2xl shadow-xl border border-slate-100 flex items-center gap-6 text-[10px] font-black tracking-widest text-slate-600 uppercase shadow-md">
             <span>Narrative Points: {Number(visibleEvents.length)}</span>
           </div>
        </div>
      </main>
      ...
      {showSaveDialog && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-6">
          <div className="bg-white rounded-[3.5rem] shadow-2xl max-w-lg w-full p-12 animate-in zoom-in-95 duration-300">
            <div className="bg-blue-50 w-16 h-16 rounded-3xl flex items-center justify-center text-blue-600 mb-8 shadow-inner">
              <Sparkles size={32} />
            </div>
            ...
            <div className="flex gap-4">
              ...
              <button onClick={saveTimeline} disabled={!saveName.trim() || loading} className="flex-[2] py-5 bg-blue-600 text-white font-bold rounded-[2rem] shadow-2xl hover:bg-blue-700 disabled:opacity-50 transition-all text-lg">
                {loading ? 'Archiving...' : 'Save to Cloud'}
              </button>
            </div>
          </div>
        </div>
